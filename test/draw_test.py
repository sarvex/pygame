...
class DrawCircleMixin:
        ...
        for pos in rect_corners_mids_and_center(
            ...
            for radius in range(max_radius + 1):
                for thickness in range(radius + 1):
                    ...
                    # drawn (it uses what is actually drawn).
                    expected_rect = create_bounding_rect(surface, surf_color, pos)
                    if expected_rect[0] < 0:
                        expected_rect[0] = 0
                    if expected_rect[1] < 0:
                        expected_rect[1] = 0
                    if expected_rect[0] > surface.get_width() - 1:
                        expected_rect[0] = surface.get_width() - 1
                    if expected_rect[1] > surface.get_height() - 1:
                        expected_rect[1] = surface.get_height() - 1
    ...
    def test_x_bounds(self):
        ...
        where = (-1e30, 80)
        bounding_rect1 = self.draw_circle(surf, color, where, radius=radius)
        self.assertEqual(bounding_rect1, pygame.Rect(0, where[1], 0, 0))
